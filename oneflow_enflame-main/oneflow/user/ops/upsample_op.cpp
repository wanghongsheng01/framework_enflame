...
REGISTER_USER_OP("upsample")
    ...
    .SetTensorDescInferFn([](user_op::InferContext* ctx) -> Maybe<void> {
      ...
      if (x_desc->shape().NumAxes() != 4) { LOG(FATAL) << "upsample only supports 4-D input"; }
      ...
      if (data_format == "channels_first") {
        ...
        *y_desc->mut_shape() = Shape(
            {x_desc->shape().At(0), static_cast<int32_t>(width_scale) * x_desc->shape().At(1),
             static_cast<int32_t>(height_scale) * x_desc->shape().At(2), x_desc->shape().At(3)});
      } else {
        UNIMPLEMENTED();
      }
