...
#include "oneflow/core/memory/memory_dev_abc_allocator.h"
...
void* MemoryAllocatorImpl::Allocate(MemoryCase mem_case, size_t size) {
  ...
  if (mem_case.has_host_mem()) {
    ...
    ptr = DevAbcMemoryAllocatorImpl::Allocate(mem_case, size);
  } else if (mem_case.has_device_enflame_mem()) {
#ifdef WITH_ENFLAME
    topsMemory_t tops_ptr = nullptr;
    OF_ENFLAME_CHECK(topsMalloc(*(Global<DTUCurrentDeviceContext>::Get()->DTUGetCtx()), &tops_ptr, size));
    ptr = tops_ptr;
#else
    UNIMPLEMENTED();
#endif  // WITH_ENFLAME
...
void MemoryAllocatorImpl::Deallocate(void* ptr, MemoryCase mem_case) {
  if (mem_case.has_host_mem()) {
    ...
    DevAbcMemoryAllocatorImpl::Deallocate(ptr, mem_case);
  } else if (mem_case.has_device_enflame_mem()) {
#ifdef WITH_ENFLAME
    OF_ENFLAME_CHECK(topsFree(*(Global<DTUCurrentDeviceContext>::Get()->DTUGetCtx()), static_cast<topsMemory_t>(ptr)));
#else
    UNIMPLEMENTED();
#endif  // WITH_ENFLAME
  } else {
