...
void AutoMemcpy(DeviceCtx* ctx, void* dst, const void* src, size_t sz,
  ...
  if (src_mem_case.has_host_mem() && dst_mem_case.has_host_mem()) {
  ...
  } else if (src_mem_case.has_device_abc_mem() || dst_mem_case.has_device_abc_mem()) {
    func = &Memcpy<DeviceType::kABC>;
    ...
    UNIMPLEMENTED();
#endif  // WITH_CUDA
  }  else if (src_mem_case.has_device_enflame_mem() || dst_mem_case.has_device_enflame_mem()) {
#ifdef WITH_ENFLAME
    if (src_mem_case.has_device_enflame_mem() && !dst_mem_case.has_device_enflame_mem()) {
      OF_ENFLAME_CHECK(topsMemcpyDeviceToHost(ctx->enflame_handle(), static_cast<topsMemory_t>(const_cast<void*>(src)), dst));
    } else if (!src_mem_case.has_device_enflame_mem() && dst_mem_case.has_device_enflame_mem()) {
      OF_ENFLAME_CHECK(topsMemcpyHostToDevice(ctx->enflame_handle(), const_cast<void*>(src), static_cast<topsMemory_t>(dst)));
    } else if (src_mem_case.has_device_enflame_mem() && dst_mem_case.has_device_enflame_mem()) {
      OF_ENFLAME_CHECK(topsMemcpy(ctx->enflame_handle(), static_cast<topsMemory_t>(const_cast<void*>(src)), static_cast<topsMemory_t>(dst)));
    } else {
      UNIMPLEMENTED();
    }
    return;
