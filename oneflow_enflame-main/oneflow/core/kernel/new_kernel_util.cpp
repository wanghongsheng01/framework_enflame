...
template<>
void Memcpy<DeviceType::kABC>(DeviceCtx* ctx, void* dst, const void* src, size_t sz) {
  if (dst == src) { return; }
  memcpy(dst, src, sz);
}

template<>
void Memset<DeviceType::kABC>(DeviceCtx* ctx, void* dst, const char value, size_t sz) {
  memset(dst, value, sz);
}
