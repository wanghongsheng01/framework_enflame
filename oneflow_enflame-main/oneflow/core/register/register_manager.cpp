...
RegstMgr::RegstMgr(const Plan& plan) {
...
#ifdef WITH_ENFLAME
      if(mem_block.mem_case().has_device_enflame_mem()) {
        topsMemory_t cur_enflame_ptr = nullptr;
        OF_ENFLAME_CHECK(topsSubMemory(static_cast<topsMemory_t>((void*)(chunk_id2ptr.at(mem_block.chunk_id()))), mem_block.chunk_offset(), mem_block.mem_size(), &cur_enflame_ptr));
        mem_block_ptr = static_cast<char*>((void*)(cur_enflame_ptr));
      } else {
  ...
  for (auto& pair : zone_id2packed_chunk) {
    ...
    if (packed_chunk->mem_case.has_device_enflame_mem()) {
      for (const MemBlockProto* block : packed_chunk->blocks) {
        char *cur_ptr = nullptr;
#ifdef WITH_ENFLAME
        topsMemory_t cur_enflame_ptr = nullptr;
        OF_ENFLAME_CHECK(topsSubMemory(static_cast<topsMemory_t>((void*)(ptr)), offset, block->mem_size(), &cur_enflame_ptr));
        cur_ptr = static_cast<char*>((void*)(cur_enflame_ptr));
#else
        UNIMPLEMENTED();
#endif
        CHECK(mem_block_id2ptr_.emplace(block->mem_block_id(), cur_ptr).second);
